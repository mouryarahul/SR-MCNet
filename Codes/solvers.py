...
def anderson_1(f, x0, m=6, lam=1e-4, threshold=50, eps=1e-3, stop_mode='rel', beta=1.0, **kwargs):
...
def anderson_2(f:Callable, x0:torch.Tensor, m:int=5, 
                lam:float=1e-4, min_iter=5, max_iter:int=50, 
                tol:float=1e-3, beta:float=1.0, verbose:bool=False
            ) -> Tuple[torch.Tensor, torch.Tensor]:
    """ Anderson acceleration for fixed point iteration. """
    bsz, d, P, Q = x0.shape
    X = torch.zeros(bsz, m, d*P*Q, dtype=x0.dtype, device=x0.device)
    F = torch.zeros(bsz, m, d*P*Q, dtype=x0.dtype, device=x0.device)
    ...
    for k in range(2, max_iter):
        ...
        if res < tol and k >= min_iter:
            break
    
    if verbose:
        print("Anderson took {:d} iterations with Residual {:4.8e}".format(k, res))
        
    return X[:,k%m].view_as(x0), res


def anderson_3(f:Callable, x0:torch.Tensor, m:int=5, 
                lam:float=1e-4, min_iter=5, max_iter:int=50, 
                tol:float=1e-3, beta:float=1.0, verbose:bool=False
            ) -> Tuple[torch.Tensor, torch.Tensor]:
    """ Anderson acceleration for fixed point iteration. """
    bsz, tensor_size = x0.shape
    X = torch.zeros(bsz, m, tensor_size, dtype=x0.dtype, device=x0.device)
    F = torch.zeros(bsz, m, tensor_size, dtype=x0.dtype, device=x0.device)
    X[:,0], F[:,0] = x0.view(bsz, -1), f(x0).view(bsz, -1)
    X[:,1], F[:,1] = F[:,0], f(F[:,0].view_as(x0)).view(bsz, -1)
    ...
    if verbose:
        print("Anderson took {} iterations with Residual {:4.8e}".format(k, res))
        
    return X[:,k%m].view_as(x0), res


def conjugate_gradient(x0, Fx, b, maxiters=25, tol=1E-8):
    x = x0.clone()
    res = b - Fx(x) # Calculate initial residuals
    pres = torch.clone(res)
    rsold = torch.sum(res*res, (-2,-1), keepdim=True)
    # Main Loop
    for i in range(maxiters):
        # if rsold.sum().item() <= tol:
        #     break
        Fx_pres = Fx(pres)
        alpha = rsold / torch.sum(pres*Fx_pres, (-2,-1), keepdim=True)
        x = x + alpha*pres
        if i > 1 and (i % 49 == 1):
            res = b - Fx(x)
        else:
        ...
        rsnew = torch.sum(res*res, (-2,-1), keepdim=True)
        if rsnew.sum().item() <= tol:
            break
...
def power_iteration(A, x, max_iter: int, tol: float):
    # Input x is a random vector
    x_prev = x / torch.linalg.vector_norm(x)
    for i in range(max_iter):
        # calculate the matrix-by-vector product Ax
        z = A(x)
        
        # re-normalize the vector
        x = z / torch.linalg.vector_norm(z)
        
        # Check for convergence
        error_norm = torch.linalg.vector_norm(x - x_prev)
        if error_norm < tol:
            break
        x_prev = x

    e = torch.sum(x * A(x)).item()

    return e, x.squeeze()

def power_iteration_mat(A, x, max_iter: int, tol: float):
    # Input x is a random vector
    x_prev = x / np.linalg.norm(x)
    for i in range(max_iter):
        # calculate the matrix-by-vector product Ax
        z = A@x
        ...
        # Check for convergence
        error_norm = np.linalg.norm(x - x_prev)
        if error_norm < tol:
            break
        x_prev = x
